...
class GestionDevoir(APIView):
    ...
    def get(self, request, id):
        ...
        if devoir:
            ...
            print(idElement)
            url2 = 'http://localhost:8000/api/v2/element' 
...
class DevoirStats(APIView):
    def get(self, request):
        result = []
        url = 'http://localhost:8000/api/v1/professeur/all'  # no trailing /
...
class DevoirElementStats(APIView):
    def get(self, request, id):
        result = []
        url1 = 'http://localhost:8000/api/v2/inscription/all'  # no trailing /
        r = http.request('GET', url1)
        data2 = json.loads(r.data)
        for j in range(len(data2)):
            if(data2[j]["id"]==id):
                url = 'http://localhost:8000/api/v2/element/all'  # no trailing /
                r = http.request('GET', url)
                data = json.loads(r.data)
